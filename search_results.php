<?php 
    if (!isset($_GET['stock'])) {
        ...
        if(array_key_exists("Error Message", $stock_numbers)){
            ...
            //find if the stock is already in the table
            $stock = strtoupper($stock_numbers["Meta Data"]["2. Symbol"]);
            ...
            $sql = "SELECT symbols.symbol FROM stocks LEFT JOIN symbols ON stocks.symbols_id = symbols.id WHERE symbols.symbol = '$stock';"; //aggregate?
...
<html lang="en">
    ...
    <body>
        <div class="container">
            <div class="row">
                <h1 class="col-12 my-4 text-center text-weight-bold">Stocks Search Results</h1>
            </div>
        </div>
        <div class="container">
            ...
            <?php else: ?>
                <div class="row mb-4">
                    <h4 class="col-4 text-center"><?php echo "Stock Name: ".strtoupper($stock_numbers["Meta Data"]["2. Symbol"])?></h4>
                    <h4 class="col-4 text-center"><?php echo "Closing price: ".$stock_numbers["Time Series (5min)"][$date]["4. close"]?></h4>
                    <p class="col-1"></p>
                    <?php if($stock_exists == 0): ?>
                        <a href="confirm_save.php?symbol=<?php echo strtoupper($stock_numbers["Meta Data"]["2. Symbol"]);?>&price=<?php echo $stock_numbers["Time Series (5min)"][$date]["4. close"];?>&date=<?php echo substr($date, 0, 10); ?>" role="button" class="col-2 btn btn-primary">Add to Saved</a> 
                    <?php endif; ?>
                </div>
                <div class="row mb-4">
